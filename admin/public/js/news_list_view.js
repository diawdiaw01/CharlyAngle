...
$('#formGuide').on('submit', function(e){
	...
	//alert(sendURL);exit();
	$.ajax({	type: "POST",	url:  base_url + "News/formAdd/",	data: sendURL,
				...
				if($('#checkque').val() == 'yes' && $('#checkqueFile').val() == 'no'){
					...
					showSuccessMessage("บันทึกข้อมูลสำเร็จ");
                         $("button.confirm").click(function () {
							 ...
							 location.href = base_url + "News/lists/" + news_category_id;
...
function loadNewsList() {
	var sendURL="cmd=loadNewsList&news_category_id=" + news_category_id;
	//alert(sendURL);
		$.ajax({	type: "POST",	url:  base_url + "News/loadNewsList" ,	data: sendURL,
			success: function(msg)
				{
...
$(function(){
 ...
 $('#myModalEdit').on('show.bs.modal', function (event) {
		...
		modal.find('.modal-body #newsFileList').html(button.data('file')+ ' <a href="#" data-id="'+button.data('id')+'" title="ลบ" class="text-danger deleteFile"><i class="fas fa-trash-alt"></i></a>');
		...
		modal.find('.modal-body #youtubeEdit').val(button.data('youtube'));
		...
		'onUploadComplete': function(file, data) {
			$('#newsFileList').html(file.name + ' <a href="#" data-id="'+$('#editID').val()+'" title="ลบ" class="text-danger deleteFile"><i class="fas fa-trash-alt"></i></a>');
...
$('#formGuideEdit').on('submit', function(e){
	...
	$.ajax({	type: "POST",	url:  base_url + "News/formEdit/",	data: sendURL,
		success: function(msg)
			{
				if (msg == "true") {
...
$( "#newsFileList" ).delegate( ".deleteFile", "click", function() {
	
	if(confirm('ต้องการลบไฟล์แนบ?')){
		$('#loadAndDisable').addClass('ld-loading');
	var sendURL="cmd=deleteFile&news_id=" + $(this).data('id');
	//alert(sendURL);
	$.ajax({	type: "POST",	url:  base_url + "News/deleteFile/",	data: sendURL,
			success: function(msg)
				{
                    $('#newsFileList').html('');
                    $('#loadAndDisable').removeClass('ld-loading');
					toastr.warning('ลบไฟล์แนบเรียบร้อยแล้ว');
...
$( function() {
   ...
   $('#Editnews_category_id').change(function(){
    ...
    //alert(sendURL);
	$.ajax({	type: "POST",	url:  base_url + "News/updateCategoryID/",	data: sendURL,
			success: function(msg)
				{
                    if(msg == "true"){
