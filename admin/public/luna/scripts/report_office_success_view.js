...
function loadEdocSetting() {
	//alert('ok');
	$('.loader-spin').removeClass('none');
	var sendURL="cmd=loadEdocSetting&month=" + $('#hiddenMonth').val() + "&year=" + $('#hiddenYear').val() + "&office=" + $('#hiddenOffice').val() + "&operation=" +  $('#hiddenOperation').val();
	//alert(sendURL);
		$.ajax({	type: "POST",	url:  base_url + "Report/loadEdocTrackingWithOffice/",	data: sendURL,
			success: function(msg)
				{
					//alert(msg);
