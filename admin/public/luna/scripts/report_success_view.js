...
function loadEdocSetting() {
	//alert('ok');
	$('.loader-spin').removeClass('none');
	var sendURL="cmd=loadEdocSetting&month=" + $('#hiddenMonth').val() + "&year=" + $('#hiddenYear').val() + "&emp_id=" + $('#hiddenPerson').val();
	//alert(sendURL);
		$.ajax({	type: "POST",	url:  base_url + "Report/loadEdocTrackingWithSuccess/",	data: sendURL,
			success: function(msg)
				{
					//alert(msg);
