...
function loadOffice() {
	$('.loader-spin').removeClass('none');
	var sendURL="cmd=loadOffice";
		$.ajax({	type: "POST",	url:  base_url + "Report/loadOffice/"+$('#hiddenOffice').val(),	data: sendURL,
			success: function(msg)
				{
					$('#loadOffice').html(msg);
					$('.loader-spin').addClass('none');
		...
		error: function()
				{
					toastr.error('มีปัญหากับการเชื่อมต่อ loadOffice');
				}
					
        });
}

$('#reportForPersonSearch').click(function(){
	window.location = base_url + "Report/office/" + $('#loadMonth').val() + "/" + $('#loadYear').val()  + "/" + $('#loadOffice').val();
});
