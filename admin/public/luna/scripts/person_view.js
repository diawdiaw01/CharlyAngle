...
function loadPerson() {
	$('.loader-spin').removeClass('none');
	var sendURL="cmd=loadPerson";
		$.ajax({	type: "POST",	url:  base_url + "Report/loadPerson/"+$('#hiddenPerson').val(),	data: sendURL,
			success: function(msg)
				{
					$('#loadPerson').html(msg);
					$('.loader-spin').addClass('none');
		...
		error: function()
				{
					toastr.error('มีปัญหากับการเชื่อมต่อ loadPerson');
				}
...
$('#reportForPersonSearch').click(function(){
	window.location = base_url + "Report/person/" + $('#loadMonth').val() + "/" + $('#loadYear').val()  + "/" + $('#loadPerson').val();
});
